...
class Solution:
    def getHint(self, secret: str, guess: str) -> str:
        bull_cnt = 0
        tmp = 0
        origin_len = len(guess)
        while tmp < origin_len and tmp < len(secret):
            if secret[tmp] == guess[tmp]:
                bull_cnt += 1
                secret = secret[:tmp] + secret[tmp+1:]
                guess = guess[:tmp] + guess[tmp+1:]
            else:
                tmp += 1
        ...
        for i in range(len(guess)):
            if guess[i] in tmp:
                ...
                tmp.add(guess[i])
                cow_cnt += min(secret.count(guess[i]), guess.count(guess[i]))
